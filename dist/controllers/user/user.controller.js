...
class UserController {
    ...
    getUserTransactions = async (req, res, next) => {
        try {
            ...
            const result = await this._userService.getUserTransactions(req.user.id, query);
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, messages_1.MESSAGES.COMMON.SUCCESS, result.data, result.pagination).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    generateWorkout = async (req, res, next) => {
        try {
            if (!req.user?.id) {
                throw new appError_1.AppError(statuscode_1.STATUS.UNAUTHORIZED, messages_1.MESSAGES.USER.USER_NOT_FOUND);
            }
            const result = await this._userService.generateWorkout(req.user.id);
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, messages_1.MESSAGES.WORKOUT_PLAN.GENERATED, result).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
                next(error);
    ...
    getWorkoutPlans = async (req, res, next) => {
        try {
            ...
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, messages_1.MESSAGES.WORKOUT_PLAN.PLAN_LIST_FETCHED, plans).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
                next(error);
    ...
    markDayCompleted = async (req, res, next) => {
        try {
            if (!req.user?.id) {
                throw new appError_1.AppError(statuscode_1.STATUS.UNAUTHORIZED, messages_1.MESSAGES.USER.USER_NOT_FOUND);
            }
            ...
            if (typeof completed !== "boolean") {
                throw new appError_1.AppError(statuscode_1.STATUS.BAD_REQUEST, "completed field is required and must be a boolean");
            }
            ...
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, messages_1.MESSAGES.WORKOUT_PLAN.DAY_MARKED, updatedPlan).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
                next(error);
    ...
    markExerciseStatus = async (req, res, next) => {
        try {
            if (!req.user?.id) {
                throw new appError_1.AppError(statuscode_1.STATUS.UNAUTHORIZED, messages_1.MESSAGES.USER.USER_NOT_FOUND);
            }
            ...
            if (status !== fitness_constant_1.WORKOUT_EXERCISE_STATUS.PENDING &&
                status !== fitness_constant_1.WORKOUT_EXERCISE_STATUS.ACTIVE &&
                status !== fitness_constant_1.WORKOUT_EXERCISE_STATUS.COMPLETED &&
                status !== fitness_constant_1.WORKOUT_EXERCISE_STATUS.SKIPPED) {
                throw new appError_1.AppError(statuscode_1.STATUS.BAD_REQUEST, "status field is required and must be PENDING, ACTIVE, COMPLETED, or SKIPPED");
            }
            ...
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, "Exercise status updated successfully", updatedPlan).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
                next(error);
    ...
    getWorkoutProgress = async (req, res, next) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new appError_1.AppError(statuscode_1.STATUS.UNAUTHORIZED, messages_1.MESSAGES.USER.USER_NOT_FOUND);
            ...
            new success_response_1.SuccessResponse(statuscode_1.STATUS.OK, messages_1.MESSAGES.WORKOUT_PLAN.PROGRESS_FETCHED, progress).send(res);
        }
        catch (error) {
            if (error instanceof Error) {
                next(error);
