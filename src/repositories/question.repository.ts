...
import { ConditionOperator, QuestionType, DataSource } from "@/constants/question.constant";

export default class QuestionRepository extends BaseRepository<OnboardingQuestion, IQuestion> implements IQuestionRepository {
  constructor() {
    super(QuestionModel);
  }
  ...
  async getQuestionById(questionId: string): Promise<OnboardingQuestion | null> {
    const doc = await QuestionModel.findById(questionId);
    return doc ? this.toInterface(doc) : null;
  }

  async updateQuestion(questionId: string, data: Partial<OnboardingQuestion>): Promise<void> {
    const doc = await QuestionModel.findById(questionId);
    if (!doc) return;
    ...
    if (data.groupId !== undefined) {
      doc.groupId = (data.groupId && mongoose.Types.ObjectId.isValid(data.groupId)) 
    ...
    if (data.dataSource !== undefined) doc.dataSource = data.dataSource as DataSource;
    ...
    if (data.next !== undefined) {
      doc.next = data.next?.map(n => {
        ...
        if (n.nextQuestionId && mongoose.Types.ObjectId.isValid(n.nextQuestionId)) {
          entry.nextQuestionId = new mongoose.Types.ObjectId(n.nextQuestionId);
        }
        return entry as { condition: { operator: ConditionOperator; value?: OnboardingValue }; nextQuestionId: mongoose.Types.ObjectId };
      });
    }
  ...
  async getAllQuestions(query: QuestionQuery): Promise<GetAllQuestionsResponse> {
  ...
  async getQuestionsByGroup(groupId: string, activeOnly: boolean = true): Promise<OnboardingQuestion[]> {
    const filter: Record<string, unknown> = { groupId: new mongoose.Types.ObjectId(groupId) };
  ...
  async toggleQuestionStatus(questionId: string): Promise<OnboardingQuestion | null> {
