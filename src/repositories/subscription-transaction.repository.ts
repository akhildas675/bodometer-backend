import { SubscriptionTransactionModel, ISubscriptionTransaction } from "@/models/subscription-transaction.model";
...
export class SubscriptionTransactionRepository implements ISubscriptionTransactionRepository {
  ...
  async findByTransactionId(transactionId: string): Promise<ISubscriptionTransaction | null> {
    return SubscriptionTransactionModel.findOne({ transactionId }).lean() as Promise<ISubscriptionTransaction | null>;
  }
}